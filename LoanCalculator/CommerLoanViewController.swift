...
class CommerLoanViewController: UIViewController {
    
    var infoTableView: UITableView!
    var commerLoan: CommerLoan!
    var dataSource: [String] = ["贷款总额(万元)","贷款时长(年)","起始时间","贷款利率(%)","贷款利率倍数","还款类型"]
    ...
    func calculate() {
        if checkIfInfoEmpty() {
            ...
            commerLoan = CommerLoan(year: Int(cell[1].infoInput.text!)!, money: Int(cell[0].infoInput.text!)!, rate: Double(cell[3].infoInput.text!)! * Double(cell[4
    ...
    func checkIfInfoEmpty() -> Bool {
        ...
        for i in 0...4 {
            if cell[i].infoInput.text == nil || cell[i].infoInput.text == "" {
                return false
            }
        }
        return true
...
extension CommerLoanViewController: UITableViewDelegate {
...
extension CommerLoanViewController: UITableViewDataSource {
...
extension CommerLoanViewController: UITextFieldDelegate {
